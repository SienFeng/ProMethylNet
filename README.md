...
This experiment uses the Uniprot dataset to obtain protein sequences and their methylation site information by reading FASTA files and TSV files, cleans the sequences (for example, replacing illegal amino acids with X and filtering sequences of inappropriate length), and generates a label for each sequence indicating the methylation sites. Next, multiple features are extracted for each valid sequence, including semantic embeddings obtained using the pre-trained ProtBERT model, evolutionary information obtained by PSI-BLAST (PSSM), One-Hot encoding, and numerical features based on the physical and chemical properties of amino acids. These features were then fused and saved as an HDF5 file. Subsequently, all data was divided into training, validation, and test sets. 5-fold cross-validation was used on the training and validation sets, and the sequences were uniformly filled or truncated in each fold. and constructed an adjacency matrix to assist the graph neural network processing. The model consists of a multi-scale convolutional neural network (MSCANet) combined with a multi-head attention mechanism, a graph attention network (GAT), a bidirectional LSTM, and a self-attention module. It is mapped to a binary classification output through a fully connected layer, and trained using a weighted binary cross-entropy loss function and a category weight balancing strategy. Early stopping and learning rate decay are used to prevent overfitting. During the experiment, not only were the accuracy, precision, recall, F1-score, AUC-ROC and MCC of each fold cross-validation recorded, but the optimal classification threshold was also optimized on the validation set. In addition, the impact of each part on the model performance was evaluated through a series of ablation experiments (such as turning off the CNN, GAT, Attention or LSTM module, or adjusting the feature combination). Finally, the final model was trained on the entire training and validation set and evaluated on the test set. Graphs such as the ROC curve, Precision-Recall curve and confusion matrix are also generated to comprehensively demonstrate the prediction effect and robustness of the model.

# Model
## Model Architecture
![Architecture Of ProMethylNet](/Users/caryallen/Desktop/Elsevier/Images/Global_structure_diagram.pdf)
